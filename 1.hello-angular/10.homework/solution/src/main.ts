...
@Component({
  ...
  template: `
    <div class="card card-block">
      ...
      <a class="btn btn-danger" (click)="deletePoem()">删除</a>
    </div>
  `,
})
class PoemComponent {
  ...
  @Output() poemDeleted = new EventEmitter<Poem>();

  deletePoem() {
    this.poemDeleted.emit(this.data);
  }
}
...
@Component({
  ...
  template: `
    ...
    <poem
      *ngFor="let p of poems"
      [poem]="p"
      (poemDeleted)="deletePoem($event)"
    ></poem>
  `,
})
